...
const Upload = () => {
   ...
   const handleDocsSubmit = (e) => {
      ...
      axios
         .post("/api/uploads/docs", formData, { headers: { "Content-Type": "application/octet-stream" } })
         .then(res => {
            setFile(null)
   ...
   const handleImagesSubmit = (e) => {
        ...
        axios.post('/api/uploads/images', { images, userObj })
            .then(res => {
                setImages([])
                setStatusMessage('✓ Successfully Submitted ✓')
                setTimeout(() => {
        ...
        { isLoading ? (
            ...
            <>
                <form
                    ...
                    <label>Upload PDF Documents (No Photos) </label>
                    <input type="file" className="input" onChange={handleFileUpload} />
                    <button type="submit" className="buttons mt-3 w-[80%]"> Submit </button> 
                </form>
                ...
                <form
                    ...
                    <label>  </label>
                    <input type="file" className="input" onChange={handleImageUpload} multiple />
                    <button type="submit" className="buttons mt-3 w-[80%]"> Submit </button> 
                    <div id="navbar" className="w-[80%] rounded mt-2 flex flex-row items-center overflow-x-auto border p-2">
